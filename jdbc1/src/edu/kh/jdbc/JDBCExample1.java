...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// java.sql.Statement 
		// - 1) SQL을 Java -> DB 에 전달 
		// - 2) DB에서 SQL 수행한 결과를 반환 받아 되돌아옴 (DB -> Java) (ex. 스쿨버스 같은 역할 / 전달해주는 객체)
		Statement stmt = null;
		...
		try {
			...
			// 2-1 ) Oracle JDBC Driver 객체를 메모리에 로드 하기(올려놓기)
			...
			// 	-> 전달한 sql이 SELECT 문일 때 결과로 ResultSet 객체 반환
			
			// 2) int Statement.executeUpdate(sql);
			// 	-> 전달하는 sql이 DML(INSERT/ UPDATE/ DELETE) 일 떄 실행 메서드
			// 	-> 결과로 int 반환 (삽입, 수정, 삭제된 행의 갯수)
			
			rs = stmt.executeQuery(sql);
			
			
			// 6. 조회 결과가 담겨있는 rs를 커서(cursor)를 이용해서 
			//  	1행 씩 접근해 각 행에 작성된 컬럼 값 얻어오기
			
			// boolean ResultSet.next() : 
			//  커서를 다음 행으로 이동 시킨 후 이동된 행에 값이 있으면 true, 없으면 false 반환
			//  맨 처음 호출 시 1행 부터 시작
			...
			//  다른 클라이언트(ex. 다른 java 프로그램)가 추가적으로 연결되지 못하는 문제 발생할 수 있음!
