...
public class UserService {
	...
	/** 1. User 등록 서비스
	 * @param user // parameter(매개변서) : 입력받은 id, pw, name 이 세팅된 객체
	...
	public int insertUser(User user) throws Exception{
		...
		int result = dao.insertUser(conn, user); // dao 가지고 있는 insertUser를 
		
		// 4. DML(INSERT) 수행 결과에 따라 트랜잭션 제어 처리(commit/rollback)
	 ...
	 * @return serchList : 조회된 User 객체들이 담긴 List
	 */
	public List<User> selectName(String keyword) throws Exception{
		...
		// 2. DAO 메서드 호출(SELECT) 후 결과반환(List<User>) 받기
		// DAO에 있는 selectName을 호출해서 반환 받는 값을 담겠다.
	 ...
	 * @param inputNO : 사용자에게 입력받은 USER_NO
	 * @return result
	 */
	public User selectUser(int inputNo) throws Exception{
		Connection conn = JDBCTemplate.getConnection();
		
		User user = dao.selectUser(conn, inputNo);
	 ...
	 * @return
	 */
	public int deleteUser(int deleteNo) throws Exception{
		
		Connection conn = JDBCTemplate.getConnection();
		
		int result = dao.deleteUser(conn, deleteNo);
	...
	/** 8. userList에 있는 모든 User 객체를 INSERT 하는 서비스
	 * @param userList
	 * @return
	 */
	public int multiInsertUser(List<User> userList) throws Exception{
		...
		// 2) Java 반복문을 이용한 다중 INSERT (우리는 이것 사용!)
